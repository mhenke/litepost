...
<cfset myFusebox.thisCircuit = "login">
<cfset myFusebox.thisFuseaction = "login">
<cfset myFusebox.trace("Runtime","&lt;include template=""act_login.cfm"" circuit=""login""/&gt;") >
<cftry>
<cfoutput><cfinclude template="../home/login/act_login.cfm"></cfoutput>
<cfcatch type="missingInclude"><cfif len(cfcatch.MissingFileName) gte 13 and right(cfcatch.MissingFileName,13) is "act_login.cfm">
<cfthrow type="fusebox.missingFuse" message="missing Fuse" detail="You tried to include a fuse act_login.cfm in circuit login which does not exist (from fuseaction login.login).">
...
<cfset variables.title = "Login Success" >
<cfif isDefined("variables.message")><cfset myFusebox.stack["variables.message"] = variables.message ></cfif>
<cfset variables.message = "You have been logged in successfully." >
